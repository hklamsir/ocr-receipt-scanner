...
// 裁剪 Modal 結構（須與 includes/receipts/crop_modal.php 保持一致）。
// 若 PHP include 因快取/同步未生效而 DOM 中缺少 #cropModal，則用 JS 即時注入，確保功能可用。
...
<div id="cropModal" class="edit-modal">
    <div class="edit-modal-content" style="max-width:90%; max-height:90vh;">
        ...
        <div class="crop-modal-body" style="padding:15px; display:flex; flex-direction:column; max-height:calc(90vh - 120px);">
            <div class="crop-container" style="flex:1; min-height:300px; max-height:60vh; background:#f5f5f5; display:flex; justify-content:center; align-items:center; overflow:hidden;">
                <img id="cropImage" style="max-width:100%; max-height:100%;">
            </div>
            <div class="crop-toolbar" style="display:flex; gap:10px; padding-top:15px; justify-content:center; flex-wrap:wrap;">
...
function ensureCropModal() {
    if (!document.getElementById('cropModal')) {
        ...
        wrap.innerHTML = CROP_MODAL_HTML.trim();
