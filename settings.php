...
try {
    ...
    $row = $stmt->fetch();
    if (!empty($row['theme']) && in_array($row['theme'], ['teal', 'elegant', 'minimal', 'dark'], true)) {
...
<div class="container">
    ...
    <div class="settings-grid">
        ...
        <!-- 外觀主題卡片 -->
        <div class="settings-card theme-manager-card">
            <div class="settings-card-header">
                <span class="settings-card-icon">🎨</span>
                <h3>外觀主題</h3>
            </div>
            <p>選擇網站配色，儲存後生效並跨裝置同步。</p>
            <div class="theme-current-preview" id="themeCurrentPreview"></div>
            <button class="btn btn-primary" onclick="openThemeModal()">選擇主題</button>
        </div>
...
// 外觀主題 Modal
include __DIR__ . '/includes/settings/theme_modal.php';
